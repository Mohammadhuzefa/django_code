...
def detail(req,pk):
    ...
    if user:
        if req.method=='PUT':
            ...
            print(p_n_data)
            print(type(p_n_data))
            serializer = dataSerializer(p_o_data,data=p_n_data) #     added p_o_data--------
            ...
            print(p_n_data)
            print(type(p_n_data))
            serializer = dataSerializer(p_o_data,data=p_n_data,partial=True) # added only partial=True--------
            if serializer.is_valid():
                print(serializer.validated_data)
                serializer.save()
...
def listt(req):
    if req.method=='POST':
        ...
        if serializer.is_valid():
            ...
            return HttpResponse(json,content_type="application/json")
    x=data.objects.all()
    ser=dataSerializer(x,many=True)
    print(ser)
    print(ser.data)
    json = JSONRenderer().render(ser.data)
    print(json)
